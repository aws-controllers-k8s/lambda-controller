...
func (r *resource) SetIdentifiers(identifier *ackv1alpha1.AWSIdentifiers) error {
	...
	r.ko.Status.UUID = &identifier.NameOrID
...
func (r *resource) PopulateResourceFromAnnotation(fields map[string]string) error {
	primaryKey, ok := fields["uuid"]
	if !ok {
		return ackerrors.NewTerminalError(fmt.Errorf("required field missing: uuid"))
	}
	r.ko.Status.UUID = &primaryKey
